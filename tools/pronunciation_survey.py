...
def cmd_merge(args: argparse.Namespace) -> None:
    ...
    wrong = [w for w, v in verdicts.items() if v == "wrong"]
    unsure = [w for w, v in verdicts.items() if v == "unsure"]
    right = [w for w, v in verdicts.items() if v == "right"]
    print(f"heard right {len(right)}, wrong {len(wrong)}, unsure {len(unsure)}"
          + (f"  ->  {len(wrong)/judged*100:.1f}% wrong" if judged else ""))
    if not wrong:
